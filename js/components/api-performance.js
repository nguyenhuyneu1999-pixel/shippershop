...
SS.ApiPerformance = {
  show: function(arg) {
    var url = '/api-performance.php' + (arg ? (typeof arg === 'number' ? '?conversation_id=' + arg + '&post_id=' + arg : '?district=' + encodeURIComponent(arg)) : '');
    SS.api.get(url).then(function(d) {
      ...
      html += '</div>';
      SS.ui.sheet({title: 'ApiPerformance', html: html});
    });
  }
};
