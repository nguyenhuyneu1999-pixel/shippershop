window.SS = window.SS || {};
SS.ConvGroupDelivery = {
  show: function(arg) {
    var url = '/conv-group-delivery.php' + (arg ? (typeof arg === 'number' ? '?conversation_id=' + arg : '?text=' + encodeURIComponent(arg)) : '');
    SS.api.get(url).then(function(d) {
      ...
      html += '</div>';
      SS.ui.sheet({title: 'ConvGroupDelivery', html: html});
    });
  }
};
