window.SS = window.SS || {};
SS.ConvIncidentReport = {
  show: function(arg) {
    var url = '/conv-incident-report.php' + (arg ? (typeof arg === 'number' ? '?conversation_id=' + arg : '?text=' + encodeURIComponent(arg)) : '');
    SS.api.get(url).then(function(d) {
      ...
      html += '</div>';
      SS.ui.sheet({title: 'ConvIncidentReport', html: html});
    });
  }
};
