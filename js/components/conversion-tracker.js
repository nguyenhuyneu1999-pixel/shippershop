window.SS = window.SS || {};
SS.ConversionTracker = {
  show: function(arg) {
    var url = '/conversion-tracker.php' + (arg ? (typeof arg === 'number' ? '?conversation_id=' + arg : '?text=' + encodeURIComponent(arg)) : '');
    SS.api.get(url).then(function(d) {
      ...
      html += '</div>';
      SS.ui.sheet({title: 'ConversionTracker', html: html});
    });
  }
};
