...
SS.ParkingFinder = {
  show: function(arg) {
    var url = '/parking-finder.php' + (arg ? (typeof arg === 'number' ? '?conversation_id=' + arg + '&post_id=' + arg : '?district=' + encodeURIComponent(arg)) : '');
    SS.api.get(url).then(function(d) {
      ...
      html += '</div>';
      SS.ui.sheet({title: 'ParkingFinder', html: html});
    });
  }
};
