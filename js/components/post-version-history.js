window.SS = window.SS || {};
SS.PostVersionHistory = {
  show: function(arg) {
    var url = '/post-version-history.php' + (arg ? (typeof arg === 'number' ? '?conversation_id=' + arg + '&post_id=' + arg : '?district=' + encodeURIComponent(arg)) : '');
    SS.api.get(url).then(function(d) {
      ...
      html += '</div>';
      SS.ui.sheet({title: 'PostVersionHistory', html: html});
    });
  }
};
